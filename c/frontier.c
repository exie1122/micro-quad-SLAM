...
// - Frontier exploration layered on top of the clean hover/landing controller
...
typedef enum {
  NAV_MODE_HOVER_ONLY = 0,
  NAV_MODE_FRONTIER_BASIC
} NavMode;

static NavMode nav_mode = NAV_MODE_FRONTIER_BASIC;

static const float    FRONTIER_GRID_CELL_M             = 0.15f;
enum {
  FRONTIER_GRID_W = 1024,
  FRONTIER_GRID_H = 1024,
  FRONTIER_GRID_CELLS = FRONTIER_GRID_W * FRONTIER_GRID_H
};
static const float    FRONTIER_SENSOR_FOV_DEG          = 70.0f;
static const float    FRONTIER_RAY_STEP_M              = 0.075f;
static const float    FRONTIER_OCC_END_MARGIN_M        = 0.05f;
static const float    FRONTIER_OCCUPIED_MAX_RANGE_M    = 3.50f;
static const float    FRONTIER_TARGET_CLEARANCE_M      = 0.35f;
static const float    FRONTIER_PATH_CLEARANCE_M        = 0.25f;
static const float    FRONTIER_STEP_MAX_M              = 0.25f;
static const float    FRONTIER_TARGET_DIST_CAP_M       = 1.20f;
static const float    FRONTIER_ARRIVAL_TOL_M           = 0.20f;
static const float    FRONTIER_MAX_DRIFT_M             = 0.80f;
static const float    FRONTIER_EMERGENCY_OBS_M         = 0.22f;
static const float    FRONTIER_MOVE_REJECT_OBS_M       = 0.30f;
static const float    FRONTIER_MAX_SELECT_SPEED_MPS    = 0.20f;
static const float    FRONTIER_MAX_SETTLE_SPEED_MPS    = 0.25f;
static const uint8_t  FRONTIER_MIN_OF_QUALITY          = 60;
static const uint32_t FRONTIER_MAX_MOVES               = 12;
static const uint32_t FRONTIER_MAX_MOVE_FAILURES       = 3;
static const float    FRONTIER_FWD_HALF_CONE_DEG       = 120.0f; // reject targets outside this half-angle from nose
static const float    FRONTIER_HEADING_PENALTY         = 0.40f;  // score penalty at cone edge (scaled by angle fraction)
static const uint32_t FRONTIER_MIN_CLUSTER_CELLS       = 2;
static const uint64_t FRONTIER_SCAN_FRESH_MS           = 800;
static const uint64_t FRONTIER_SCAN_HOVER_MS           = 1200;
static const uint64_t FRONTIER_SETTLE_MS               = 1500;
static const uint64_t FRONTIER_MOVE_TIMEOUT_MS         = 7000;
static const uint64_t FRONTIER_MOVE_DIAG_MS            = 500;
static const uint64_t FRONTIER_PROGRESS_CHECK_MS       = 2000;   // check error progress interval
static const float    FRONTIER_PROGRESS_MIN_GAIN_M     = 0.02f;  // min error reduction per check interval
static const uint64_t FRONTIER_MAX_TIME_MS             = 60000;
static const uint64_t FRONTIER_SENSOR_ABORT_MS         = 2500;

// ----------------------------- Air-wall repulsion -------------------------
// Applies in ALL airborne hover-like states to prevent wall contact during
// prelock drift.  Uses filtered ToF readings in body frame, rotated to NED.
static const float    AIRWALL_ENGAGE_M     = 0.35f;  // start repulsion
static const float    AIRWALL_HARD_M       = 0.15f;  // full repulsion
static const float    AIRWALL_MAX_VEL_MPS  = 0.30f;  // peak push-back speed
static const uint64_t AIRWALL_LOG_MS       = 1000;   // log throttle interval
...
static const float    TAKEOFF_BRAKE_VXY_MAX_MPS  = 0.18f;  // max lateral velocity during brake (must be < VY_LOCK_MAX_MPS=0.20)
...
static const char* state_name(State s) {
  switch (s) {
    ...
    case ST_HOVER_SCAN: return "HOVER_SCAN";
    case ST_SELECT_FRONTIER: return "SELECT_FRONTIER";
    case ST_MOVE_TO_FRONTIER: return "MOVE_TO_FRONTIER";
...
static bool state_is_hover_like(State s) {
  return s == ST_HOVER ||
         s == ST_HOVER_SCAN ||
         s == ST_SELECT_FRONTIER ||
         s == ST_MOVE_TO_FRONTIER ||
         s == ST_SETTLE;
}

static bool state_is_frontier(State s) {
  return s == ST_HOVER_SCAN ||
         s == ST_SELECT_FRONTIER ||
         s == ST_MOVE_TO_FRONTIER ||
         s == ST_SETTLE;
}
...
static int8_t   frontier_grid[FRONTIER_GRID_H][FRONTIER_GRID_W];
static bool     frontier_grid_initialized = false;
static float    frontier_grid_origin_x_m = NAN;
static float    frontier_grid_origin_y_m = NAN;
static uint64_t frontier_last_map_scan_ms = 0;
static uint64_t frontier_cycle_start_ms = 0;
static uint64_t frontier_state_enter_ms = 0;
static uint32_t frontier_move_count = 0;
static uint32_t frontier_move_fail_count = 0;
static float    frontier_target_x_m = NAN;
static float    frontier_target_y_m = NAN;
static float    frontier_move_start_x_m = NAN;
static float    frontier_move_start_y_m = NAN;
static uint64_t frontier_last_move_diag_ms = 0;
static float    frontier_progress_err_m = NAN;     // error at last progress checkpoint
static uint64_t frontier_progress_check_ms = 0;    // timestamp of last progress checkpoint
static uint32_t frontier_last_cluster_cells = 0;
static float    frontier_last_cluster_score = NAN;
...
static bool scan_fresh(uint64_t t) {
  return have_scan_frame && last_scan_host_ms != 0 && (t - last_scan_host_ms) < FRONTIER_SCAN_FRESH_MS;
}
...
static HoverPhase current_hover_phase(void) {
  ...
  if (state_is_hover_like(st) && hover_xy_locked) return HPH_FULL_HOVER;
  if (state_is_hover_like(st) && xy_lock_candidate_active) return HPH_XY_LOCK;
  if (state_is_hover_like(st)) return HPH_PRELOCK_HOVER;
...
static float logged_z_target_down(void) {
  ...
  if (st == ST_MOVE_TO_FRONTIER) return -HOVER_TARGET_M;
  if (state_is_hover_like(st)) return hover_target_z_down();
  return NAN;
}
...
// Compute air-wall repulsion velocity in NED frame from body-frame ToF.
// Returns true if any sensor is within the engage distance.
static bool airwall_repulsion(uint64_t t, float *vx_ned, float *vy_ned) {
  *vx_ned = 0.0f;
  *vy_ned = 0.0f;

  if (!have_att || !scan_fresh(t)) return false;

  static const float dir_body_rad[4] = {
    0.0f,                           // D_FRONT
    (float)M_PI * 0.5f,             // D_RIGHT
    (float)M_PI,                    // D_BACK
    (float)M_PI * -0.5f             // D_LEFT
  };

  bool active = false;
  for (int d = 0; d < NUM_SENSORS; d++) {
    float dist = tof_filt_m[d];
    if (!isfinite(dist) || dist >= AIRWALL_ENGAGE_M) continue;

    float frac = (AIRWALL_ENGAGE_M - dist) / (AIRWALL_ENGAGE_M - AIRWALL_HARD_M);
    if (frac > 1.0f) frac = 1.0f;
    if (frac < 0.0f) frac = 0.0f;
    float mag = frac * AIRWALL_MAX_VEL_MPS;

    // Sensor faces body direction; push AWAY (opposite) in NED
    float sensor_ned_rad = yaw_rad + dir_body_rad[d];
    *vx_ned -= mag * cosf(sensor_ned_rad);
    *vy_ned -= mag * sinf(sensor_ned_rad);
    active = true;
  }

  if (active) {
    static uint64_t last_airwall_log = 0;
    if (t - last_airwall_log >= AIRWALL_LOG_MS) {
      last_airwall_log = t;
      printf("AIRWALL: vx=%.2f vy=%.2f  F=%.2f R=%.2f B=%.2f L=%.2f\n",
             *vx_ned, *vy_ned,
             tof_filt_m[D_FRONT], tof_filt_m[D_RIGHT],
             tof_filt_m[D_BACK], tof_filt_m[D_LEFT]);
      if (txt_log_fp)
        fprintf(txt_log_fp, "[%.3f] AIRWALL: vx=%.2f vy=%.2f  F=%.2f R=%.2f B=%.2f L=%.2f\n",
                t * 0.001f, *vx_ned, *vy_ned,
                tof_filt_m[D_FRONT], tof_filt_m[D_RIGHT],
                tof_filt_m[D_BACK], tof_filt_m[D_LEFT]);
    }
  }
...
skip_xy_lock: (void)0;
  ...
  // Air-wall: override to velocity mode when near a wall
  float aw_vx = 0.0f, aw_vy = 0.0f;
  bool airwall_active = airwall_repulsion(t, &aw_vx, &aw_vy);

  if (airwall_active) {
    // Near a wall — always use velocity mode with repulsion, regardless
    // of XY lock state, so drift toward a wall is actively resisted.
    send_prelock_hold(aw_vx, aw_vy);
  } else if (hover_xy_locked) {
...
// ----------------------------- Requests --------------------------------
static bool hover_prelock_guard_tick(uint64_t t) {
  if (hover_xy_locked) return false;
...
// ----------------------------- Frontier exploration --------------------
static void frontier_reset_map(float center_x_m, float center_y_m) {
  for (int y = 0; y < FRONTIER_GRID_H; y++) {
    for (int x = 0; x < FRONTIER_GRID_W; x++) {
      frontier_grid[y][x] = OCC_UNKNOWN;
    }
  }

  frontier_grid_origin_x_m = center_x_m - 0.5f * (float)FRONTIER_GRID_W * FRONTIER_GRID_CELL_M;
  frontier_grid_origin_y_m = center_y_m - 0.5f * (float)FRONTIER_GRID_H * FRONTIER_GRID_CELL_M;
  frontier_grid_initialized = true;
  frontier_last_map_scan_ms = 0;
}

static void frontier_reset_session(uint64_t t) {
  frontier_cycle_start_ms = t;
  frontier_state_enter_ms = t;
  frontier_move_count = 0;
  frontier_move_fail_count = 0;
  frontier_target_x_m = NAN;
  frontier_target_y_m = NAN;
  frontier_move_start_x_m = NAN;
  frontier_move_start_y_m = NAN;
  frontier_last_move_diag_ms = 0;
  frontier_last_cluster_cells = 0;
  frontier_last_cluster_score = NAN;
}

static bool frontier_world_to_cell(float x_m, float y_m, int* out_ix, int* out_iy) {
  if (!frontier_grid_initialized) return false;
  if (!isfinite(x_m) || !isfinite(y_m)) return false;

  float fx = (x_m - frontier_grid_origin_x_m) / FRONTIER_GRID_CELL_M;
  float fy = (y_m - frontier_grid_origin_y_m) / FRONTIER_GRID_CELL_M;
  int ix = (int)floorf(fx);
  int iy = (int)floorf(fy);
  if (ix < 0 || iy < 0 || ix >= FRONTIER_GRID_W || iy >= FRONTIER_GRID_H) return false;
...
static void frontier_cell_to_world(int ix, int iy, float* out_x_m, float* out_y_m) {
  if (out_x_m) *out_x_m = frontier_grid_origin_x_m + ((float)ix + 0.5f) * FRONTIER_GRID_CELL_M;
  if (out_y_m) *out_y_m = frontier_grid_origin_y_m + ((float)iy + 0.5f) * FRONTIER_GRID_CELL_M;
}

static OccupancyCell frontier_cell_value(int ix, int iy) {
  if (ix < 0 || iy < 0 || ix >= FRONTIER_GRID_W || iy >= FRONTIER_GRID_H) return OCC_OCCUPIED;
  return (OccupancyCell)frontier_grid[iy][ix];
}

static void frontier_set_cell_world(float x_m, float y_m, OccupancyCell v) {
  int ix = 0, iy = 0;
  if (!frontier_world_to_cell(x_m, y_m, &ix, &iy)) return;
  frontier_grid[iy][ix] = (int8_t)v;
}

static float frontier_dir_base_deg(int dir) {
  switch (dir) {
    case D_FRONT: return 0.0f;
    case D_RIGHT: return 90.0f;
    case D_BACK:  return 180.0f;
    case D_LEFT:  return -90.0f;
    default:      return 0.0f;
  }
}

static void frontier_mark_ray(float x0_m, float y0_m, float yaw_deg, float dist_m) {
  if (!isfinite(x0_m) || !isfinite(y0_m) || !isfinite(yaw_deg) || !isfinite(dist_m)) return;
  if (dist_m <= 0.02f) return;

  float free_limit_m = dist_m - FRONTIER_OCC_END_MARGIN_M;
  if (free_limit_m < 0.0f) free_limit_m = 0.0f;

  for (float s = 0.0f; s <= free_limit_m; s += FRONTIER_RAY_STEP_M) {
    float x_m = x0_m + cosf(deg2rad(yaw_deg)) * s;
    float y_m = y0_m + sinf(deg2rad(yaw_deg)) * s;
    frontier_set_cell_world(x_m, y_m, OCC_FREE);
  }

  if (dist_m <= FRONTIER_OCCUPIED_MAX_RANGE_M) {
    float end_x_m = x0_m + cosf(deg2rad(yaw_deg)) * dist_m;
    float end_y_m = y0_m + sinf(deg2rad(yaw_deg)) * dist_m;
    frontier_set_cell_world(end_x_m, end_y_m, OCC_OCCUPIED);
  }
}

static void frontier_update_map_from_latest_scan(void) {
  if (nav_mode != NAV_MODE_FRONTIER_BASIC) return;

  // Diagnostic: log why scan incorporation fails (once per second)
  static uint64_t last_scan_diag_ms = 0;
  uint64_t tnow = now_ms();
  bool diag = (tnow - last_scan_diag_ms) >= 1000;

  if (!frontier_grid_initialized || !have_scan_frame || last_scan_host_ms == 0) {
    if (diag) {
      last_scan_diag_ms = tnow;
      if (txt_log_fp) fprintf(txt_log_fp,
        "[%.3f] SCAN_DIAG: grid_init=%d have_frame=%d scan_ms=%llu\n",
        tnow*0.001f, frontier_grid_initialized, have_scan_frame,
        (unsigned long long)last_scan_host_ms);
    }
    return;
  }
  if (last_scan_host_ms == frontier_last_map_scan_ms) return;
  if (!pose_valid_for_mapping(last_scan_host_ms)) {
    if (diag) {
      last_scan_diag_ms = tnow;
      if (txt_log_fp) fprintf(txt_log_fp,
        "[%.3f] SCAN_DIAG: pose_invalid scan_age=%llums\n",
        tnow*0.001f, (unsigned long long)(tnow - last_scan_host_ms));
    }
    return;
  }

  pose_sample_t pose;
  if (!pose_ring_sample_at(last_scan_host_ms, &pose)) {
    if (diag) {
      last_scan_diag_ms = tnow;
      if (txt_log_fp) fprintf(txt_log_fp,
        "[%.3f] SCAN_DIAG: pose_ring_miss scan_age=%llums\n",
        tnow*0.001f, (unsigned long long)(tnow - last_scan_host_ms));
    }
    return;
  }
  if (!pose.valid_xy || !pose.valid_att) {
    if (diag) {
      last_scan_diag_ms = tnow;
      if (txt_log_fp) fprintf(txt_log_fp,
        "[%.3f] SCAN_DIAG: pose_flags xy=%d att=%d\n",
        tnow*0.001f, pose.valid_xy, pose.valid_att);
    }
    return;
  }
  if (!isfinite(pose.x_m) || !isfinite(pose.y_m) || !isfinite(pose.yaw_deg)) return;

  const uint8_t* p = last_scan_grid_raw;
  for (int dir = 0; dir < NUM_SENSORS; dir++) {
    const uint8_t* grid = p + (dir * GRID_SIZE * 2);
    float base_deg = pose.yaw_deg + frontier_dir_base_deg(dir);

    for (int col = 0; col < TOF_COLS; col++) {
      float dist_m = robust_col_dist_m(grid, col);
      if (!isfinite(dist_m)) continue;

      float u = ((float)col + 0.5f) / (float)TOF_COLS;
      float col_offset_deg = (u - 0.5f) * FRONTIER_SENSOR_FOV_DEG;
      frontier_mark_ray(pose.x_m, pose.y_m, base_deg + col_offset_deg, dist_m);
    }
  }

  frontier_last_map_scan_ms = last_scan_host_ms;
}

static bool frontier_cell_has_unknown_neighbor(int ix, int iy) {
  for (int dy = -1; dy <= 1; dy++) {
    for (int dx = -1; dx <= 1; dx++) {
      if (dx == 0 && dy == 0) continue;
      if (frontier_cell_value(ix + dx, iy + dy) == OCC_UNKNOWN) return true;
    }
  }
  return false;
}

static bool frontier_is_frontier_cell(int ix, int iy) {
  if (frontier_cell_value(ix, iy) != OCC_FREE) return false;
  return frontier_cell_has_unknown_neighbor(ix, iy);
}

static float frontier_cell_clearance_m(int ix, int iy) {
  float best_m = NAN;
  int max_r = (int)ceilf(1.5f / FRONTIER_GRID_CELL_M);

  for (int y = iy - max_r; y <= iy + max_r; y++) {
    for (int x = ix - max_r; x <= ix + max_r; x++) {
      if (frontier_cell_value(x, y) != OCC_OCCUPIED) continue;
      float dx_m = ((float)x - (float)ix) * FRONTIER_GRID_CELL_M;
      float dy_m = ((float)y - (float)iy) * FRONTIER_GRID_CELL_M;
      float d_m = hypotf(dx_m, dy_m);
      if (isnan(best_m) || d_m < best_m) best_m = d_m;
    }
  }

  return isnan(best_m) ? 10.0f : best_m;
}

static float frontier_world_clearance_m(float x_m, float y_m) {
  int ix = 0, iy = 0;
  if (!frontier_world_to_cell(x_m, y_m, &ix, &iy)) return NAN;
  return frontier_cell_clearance_m(ix, iy);
}

static bool frontier_line_clear(float x0_m, float y0_m, float x1_m, float y1_m, float min_clearance_m) {
  float dx_m = x1_m - x0_m;
  float dy_m = y1_m - y0_m;
  float dist_m = hypotf(dx_m, dy_m);
  int steps = (int)ceilf(dist_m / FRONTIER_RAY_STEP_M);
  if (steps < 1) steps = 1;

  for (int i = 0; i <= steps; i++) {
    float u = (float)i / (float)steps;
    float x_m = x0_m + dx_m * u;
    float y_m = y0_m + dy_m * u;
    int ix = 0, iy = 0;
    if (!frontier_world_to_cell(x_m, y_m, &ix, &iy)) return false;
    if (frontier_cell_value(ix, iy) == OCC_OCCUPIED) return false;
    float clearance_m = frontier_cell_clearance_m(ix, iy);
    if (isfinite(clearance_m) && clearance_m < min_clearance_m) return false;
  }

  return true;
}

static bool frontier_build_step_target(float cur_x_m, float cur_y_m,
                                       float raw_x_m, float raw_y_m,
                                       float* out_x_m, float* out_y_m) {
  float dx_m = raw_x_m - cur_x_m;
  float dy_m = raw_y_m - cur_y_m;
  float raw_dist_m = hypotf(dx_m, dy_m);
  if (raw_dist_m < FRONTIER_ARRIVAL_TOL_M) return false;

  float step_m = raw_dist_m;
  if (step_m > FRONTIER_TARGET_DIST_CAP_M) step_m = FRONTIER_TARGET_DIST_CAP_M;
  if (step_m > FRONTIER_STEP_MAX_M) step_m = FRONTIER_STEP_MAX_M;

  while (step_m >= FRONTIER_ARRIVAL_TOL_M) {
    float tx_m = cur_x_m + dx_m * (step_m / raw_dist_m);
    float ty_m = cur_y_m + dy_m * (step_m / raw_dist_m);
    int ix = 0, iy = 0;
    if (!frontier_world_to_cell(tx_m, ty_m, &ix, &iy)) {
      step_m -= FRONTIER_GRID_CELL_M;
      continue;
    }
    if (frontier_cell_value(ix, iy) != OCC_FREE) {
      step_m -= FRONTIER_GRID_CELL_M;
      continue;
    }
    float clearance_m = frontier_cell_clearance_m(ix, iy);
    if (clearance_m < FRONTIER_TARGET_CLEARANCE_M) {
      step_m -= FRONTIER_GRID_CELL_M;
      continue;
    }
    if (!frontier_line_clear(cur_x_m, cur_y_m, tx_m, ty_m, FRONTIER_PATH_CLEARANCE_M)) {
      step_m -= FRONTIER_GRID_CELL_M;
      continue;
    }

    if (out_x_m) *out_x_m = tx_m;
    if (out_y_m) *out_y_m = ty_m;
    return true;
  }
...
static bool frontier_motion_ready(uint64_t t, bool require_low_speed, char* reasons, size_t reasons_sz) {
  if (reasons && reasons_sz > 0) reasons[0] = '\0';

  if (!fc_link_fresh(t)) append_reason(reasons, reasons_sz, "fc_link_stale");
  if (!att_fresh(t)) append_reason(reasons, reasons_sz, "att_stale");
  if (!lpos_fresh(t)) append_reason(reasons, reasons_sz, "lpos_stale");
  if (!scan_fresh(t)) append_reason(reasons, reasons_sz, "scan_stale");
  if (!pose_valid_for_mapping(t)) append_reason(reasons, reasons_sz, "pose_invalid");
  if (!xy_ctrl_ok(t)) append_reason(reasons, reasons_sz, "xy_ctrl_unhealthy");
  if (!z_ctrl_ok(t)) append_reason(reasons, reasons_sz, "z_ctrl_unhealthy");
  if (!of_fresh(t)) append_reason(reasons, reasons_sz, "flow_stale");
  if (of_fresh(t) && of_quality < FRONTIER_MIN_OF_QUALITY) append_reason(reasons, reasons_sz, "flow_q_low");
  if (!isfinite(lpos_x_m) || !isfinite(lpos_y_m)) append_reason(reasons, reasons_sz, "pos_nan");
  if (!isfinite(alt_est_m)) append_reason(reasons, reasons_sz, "alt_nan");
  if (require_low_speed && isfinite(lpos_vx_mps) && isfinite(lpos_vy_mps) &&
      hypotf(lpos_vx_mps, lpos_vy_mps) > FRONTIER_MAX_SELECT_SPEED_MPS) {
    append_reason(reasons, reasons_sz, "speed_high");
  }

  return !reasons || reasons[0] == '\0';
}

static bool frontier_abort_reason(uint64_t t, char* reason, size_t reason_sz) {
  if (reason && reason_sz > 0) reason[0] = '\0';
  if (!state_is_frontier(st)) return false;

  if (frontier_cycle_start_ms != 0 && (t - frontier_cycle_start_ms) >= FRONTIER_MAX_TIME_MS) {
    append_reason(reason, reason_sz, "max_explore_time");
    return true;
  }

  if (frontier_move_count >= FRONTIER_MAX_MOVES) {
    append_reason(reason, reason_sz, "max_moves");
    return true;
  }

  if (!fc_link_fresh(t) || !att_fresh(t) || !lpos_fresh(t) ||
      !pose_valid_for_mapping(t) || !xy_ctrl_ok(t) || !z_ctrl_ok(t) ||
      !of_fresh(t) || of_quality < FRONTIER_MIN_OF_QUALITY) {
    append_reason(reason, reason_sz, "nav_quality_bad");
    return true;
  }

  // Don't abort for scan_stale while the drone is still recovering from
  // ceiling overshoot.  "Settling" = still above target altitude AND descending.
  // Keep resetting the timer during ceiling/settling so the full timeout
  // applies only after the drone has actually reached hover altitude.
  bool alt_settling = !ceiling_active && !isnan(lpos_vz_mps) &&
                      lpos_vz_mps > 0.15f && !isnan(alt_est_m) &&
                      alt_est_m > TAKEOFF_TARGET_M + 0.05f;
  if (ceiling_active || alt_settling) {
    frontier_state_enter_ms = t;   // keep pushing the timer forward
  }
  if ((!scan_fresh(t) || frontier_last_map_scan_ms == 0) &&
      !ceiling_active && !alt_settling &&
      (t - frontier_state_enter_ms) >= FRONTIER_SENSOR_ABORT_MS) {
    append_reason(reason, reason_sz, "scan_stale");
    return true;
  }

  if (st == ST_MOVE_TO_FRONTIER && scan_fresh(t)) {
    for (int dir = 0; dir < NUM_SENSORS; dir++) {
      if (isfinite(tof_filt_m[dir]) && tof_filt_m[dir] < FRONTIER_EMERGENCY_OBS_M) {
        append_reason(reason, reason_sz, "obstacle_too_close");
        return true;
      }
    }
  }

  if (st == ST_MOVE_TO_FRONTIER &&
      isfinite(frontier_move_start_x_m) && isfinite(frontier_move_start_y_m) &&
      isfinite(lpos_x_m) && isfinite(lpos_y_m)) {
    float drift_m = hypotf(lpos_x_m - frontier_move_start_x_m,
                           lpos_y_m - frontier_move_start_y_m);
    if (drift_m > (FRONTIER_STEP_MAX_M + FRONTIER_MAX_DRIFT_M)) {
      append_reason(reason, reason_sz, "drift_excessive");
      return true;
    }
  }
...
static bool frontier_select_target(float cur_x_m, float cur_y_m,
                                   FrontierCluster* out_cluster,
                                   char* reject_reason, size_t reject_reason_sz) {
  if (reject_reason && reject_reason_sz > 0) reject_reason[0] = '\0';
  if (!frontier_grid_initialized) {
    append_reason(reject_reason, reject_reason_sz, "map_uninitialized");
    return false;
  }

  static uint8_t visited[FRONTIER_GRID_H][FRONTIER_GRID_W];
  static int queue_x[FRONTIER_GRID_CELLS];
  static int queue_y[FRONTIER_GRID_CELLS];
  static int cluster_x[FRONTIER_GRID_CELLS];
  static int cluster_y[FRONTIER_GRID_CELLS];
  memset(visited, 0, sizeof(visited));

  bool found = false;
  FrontierCluster best = {0};
  uint32_t cluster_count = 0;
  uint32_t frontier_cell_count = 0;

  for (int iy = 1; iy < FRONTIER_GRID_H - 1; iy++) {
    for (int ix = 1; ix < FRONTIER_GRID_W - 1; ix++) {
      if (visited[iy][ix] || !frontier_is_frontier_cell(ix, iy)) continue;
      ...
      while (q_head < q_tail) {
        ...
        cluster_x[cluster_n] = cx;
        cluster_y[cluster_n] = cy;
        cluster_n++;
        frontier_cell_count++;

        float wx_m = NAN, wy_m = NAN;
        frontier_cell_to_world(cx, cy, &wx_m, &wy_m);
        sum_x_m += wx_m;
        sum_y_m += wy_m;

        for (int ny = cy - 1; ny <= cy + 1; ny++) {
          for (int nx = cx - 1; nx <= cx + 1; nx++) {
            if (nx < 1 || ny < 1 || nx >= FRONTIER_GRID_W - 1 || ny >= FRONTIER_GRID_H - 1) continue;
            if (visited[ny][nx] || !frontier_is_frontier_cell(nx, ny)) continue;
            visited[ny][nx] = 1;
            queue_x[q_tail] = nx;
            queue_y[q_tail] = ny;
            q_tail++;
      ...
      if ((uint32_t)cluster_n < FRONTIER_MIN_CLUSTER_CELLS) continue;

      float centroid_x_m = sum_x_m / (float)cluster_n;
      float centroid_y_m = sum_y_m / (float)cluster_n;
      bool cluster_found = false;
      float best_raw_x_m = NAN;
      float best_raw_y_m = NAN;
      float best_raw_clearance_m = NAN;
      float best_raw_dist_m = NAN;
      float best_raw_score = NAN;
      ...
      for (int i = 0; i < cluster_n; i++) {
        ...
        float wx_m = NAN, wy_m = NAN;
        frontier_cell_to_world(cx, cy, &wx_m, &wy_m);
        float clearance_m = frontier_cell_clearance_m(cx, cy);
        if (clearance_m < FRONTIER_TARGET_CLEARANCE_M) continue;
        if (!frontier_line_clear(cur_x_m, cur_y_m, wx_m, wy_m, FRONTIER_PATH_CLEARANCE_M)) continue;

        float dist_m = hypotf(wx_m - cur_x_m, wy_m - cur_y_m);

        // Forward-cone filter: reject targets behind the drone's nose
        if (have_att && dist_m > 0.01f) {
          float bearing_rad = atan2f(wy_m - cur_y_m, wx_m - cur_x_m);
          float offset_deg = fabsf(wrap_deg(rad2deg(bearing_rad - yaw_rad)));
          if (offset_deg > FRONTIER_FWD_HALF_CONE_DEG) continue;
        }

        float centroid_err_m = hypotf(wx_m - centroid_x_m, wy_m - centroid_y_m);
        float score = dist_m + 0.35f * centroid_err_m - 0.25f * fminf(clearance_m, 1.0f);

        // Heading penalty: prefer targets closer to the nose
        if (have_att && dist_m > 0.01f) {
          float bearing_rad = atan2f(wy_m - cur_y_m, wx_m - cur_x_m);
          float offset_deg = fabsf(wrap_deg(rad2deg(bearing_rad - yaw_rad)));
          score += FRONTIER_HEADING_PENALTY * (offset_deg / FRONTIER_FWD_HALF_CONE_DEG);
        }

        if (!cluster_found || score < best_raw_score) {
          cluster_found = true;
          best_raw_x_m = wx_m;
          best_raw_y_m = wy_m;
          best_raw_clearance_m = clearance_m;
          best_raw_dist_m = dist_m;
      ...
      float step_x_m = NAN, step_y_m = NAN;
      if (!frontier_build_step_target(cur_x_m, cur_y_m, best_raw_x_m, best_raw_y_m,
                                      &step_x_m, &step_y_m)) {
        continue;
      }

      int step_ix = 0, step_iy = 0;
      if (!frontier_world_to_cell(step_x_m, step_y_m, &step_ix, &step_iy)) continue;

      float step_clearance_m = frontier_cell_clearance_m(step_ix, step_iy);
      float step_dist_m = hypotf(step_x_m - cur_x_m, step_y_m - cur_y_m);
      float step_score = step_dist_m - 0.25f * fminf(step_clearance_m, 1.0f);

      if (!found || step_score < best.score) {
        found = true;
        best.cell_count = cluster_n;
        best.centroid_x_m = centroid_x_m;
        best.centroid_y_m = centroid_y_m;
        best.target_x_m = step_x_m;
        best.target_y_m = step_y_m;
        best.clearance_m = step_clearance_m;
        best.distance_m = step_dist_m;
        best.score = step_score;
      }
    }
  }

  if (!found) {
    append_reason(reject_reason, reject_reason_sz, "no_valid_frontier");
    printf("FRONTIER: no valid frontier clusters=%u frontier_cells=%u\n",
           (unsigned)cluster_count, (unsigned)frontier_cell_count);
    return false;
  }

  frontier_last_cluster_cells = (uint32_t)best.cell_count;
  frontier_last_cluster_score = best.score;
  printf("FRONTIER: chosen cluster cells=%d centroid=(%.2f,%.2f) target=(%.2f,%.2f) dist=%.2f clr=%.2f score=%.2f\n",
         best.cell_count,
         best.centroid_x_m, best.centroid_y_m,
         best.target_x_m, best.target_y_m,
         best.distance_m, best.clearance_m, best.score);

  if (out_cluster) *out_cluster = best;
  return true;
}

static void frontier_reset_move_target(void) {
  frontier_target_x_m = NAN;
  frontier_target_y_m = NAN;
  frontier_move_start_x_m = NAN;
  frontier_move_start_y_m = NAN;
  frontier_last_move_diag_ms = 0;
  frontier_progress_err_m = NAN;
  frontier_progress_check_ms = 0;
}

static void frontier_restart_relock_window(uint64_t t, float anchor_x_m, float anchor_y_m) {
  hover_xy_locked = false;
  hover_xy_lock_ms = 0;
  hover_enter_ms = t;
  xy_lock_candidate_active = false;
  ...
  if (isfinite(anchor_x_m) && isfinite(anchor_y_m)) {
    hover_lock_x_m = anchor_x_m;
    hover_lock_y_m = anchor_y_m;
  } else if (lpos_fresh(t) && isfinite(lpos_x_m) && isfinite(lpos_y_m)) {
    hover_lock_x_m = lpos_x_m;
    hover_lock_y_m = lpos_y_m;
  } else {
    hover_lock_x_m = NAN;
...
static void frontier_log_move_progress(uint64_t t) {
  if (!isfinite(frontier_target_x_m) || !isfinite(frontier_target_y_m)) return;
  if (frontier_last_move_diag_ms != 0 &&
      (t - frontier_last_move_diag_ms) < FRONTIER_MOVE_DIAG_MS) {
    return;
  }

  frontier_last_move_diag_ms = t;

  if (!(lpos_fresh(t) && isfinite(lpos_x_m) && isfinite(lpos_y_m) &&
        isfinite(lpos_vx_mps) && isfinite(lpos_vy_mps))) {
    printf("FRONTIER: move progress pos_unavailable target=(%.2f,%.2f)\n",
           frontier_target_x_m, frontier_target_y_m);
    return;
  }

  float err_m = hypotf(lpos_x_m - frontier_target_x_m,
                       lpos_y_m - frontier_target_y_m);
  float speed_mps = hypotf(lpos_vx_mps, lpos_vy_mps);
  printf("FRONTIER: move progress err=%.2f speed=%.2f pos=(%.2f,%.2f) target=(%.2f,%.2f)\n",
         err_m, speed_mps,
         lpos_x_m, lpos_y_m,
         frontier_target_x_m, frontier_target_y_m);
}

static void frontier_tick(uint64_t t) {
  frontier_update_map_from_latest_scan();

  float yaw_deg = have_yaw_target ? yaw_target_deg : (have_att ? current_heading_deg() : 0.0f);
  float move_z_down = -HOVER_TARGET_M;

  switch (st) {
    case ST_HOVER_SCAN: {
      hover_hold_tick(t);
      if (hover_prelock_guard_tick(t)) break;

      if (!hover_xy_locked) break;
      if ((t - frontier_state_enter_ms) < FRONTIER_SCAN_HOVER_MS) break;
      if (!scan_fresh(t) || frontier_last_map_scan_ms == 0) break;

      char reasons[160];
      if (!frontier_motion_ready(t, true, reasons, sizeof(reasons))) break;

      enter_state(ST_SELECT_FRONTIER, "Scan Ready");
    } break;

    case ST_SELECT_FRONTIER: {
      hover_hold_tick(t);
      if (hover_prelock_guard_tick(t)) break;

      if (!hover_xy_locked) {
        enter_state(ST_HOVER_SCAN, "Need XY Lock");
        break;
      }
      ...
      char reject_reason[160];
      if (!frontier_select_target(lpos_x_m, lpos_y_m, &cluster, reject_reason, sizeof(reject_reason))) {
        printf("FRONTIER: ending exploration reason=%s\n",
               reject_reason[0] ? reject_reason : "no_valid_frontier");
        enter_state(ST_LANDING, "No Frontier");
        break;
      }

      frontier_target_x_m = cluster.target_x_m;
      frontier_target_y_m = cluster.target_y_m;
      frontier_move_start_x_m = lpos_x_m;
      frontier_move_start_y_m = lpos_y_m;
      frontier_last_move_diag_ms = 0;
      hover_xy_locked = false;
      xy_lock_candidate_active = false;
      xy_lock_candidate_since_ms = 0;

      printf("FRONTIER: move target=(%.2f,%.2f) dist=%.2f move#%u\n",
             frontier_target_x_m, frontier_target_y_m,
             cluster.distance_m, (unsigned)frontier_move_count);
      enter_state(ST_MOVE_TO_FRONTIER, "Frontier Selected");
    } break;

    case ST_MOVE_TO_FRONTIER: {
      if (!isfinite(frontier_target_x_m) || !isfinite(frontier_target_y_m)) {
        printf("FRONTIER: invalid target -> LANDING\n");
        enter_state(ST_LANDING, "Invalid Target");
        break;
      }

      send_pos_yaw_ned(frontier_target_x_m, frontier_target_y_m, move_z_down, yaw_deg);
      frontier_log_move_progress(t);

      // Progress gate: abort early if error is not shrinking.
      // Use a rolling baseline: track best (lowest) error seen, and abort
      // if the current error exceeds the best by a margin (diverging).
      if (lpos_fresh(t) && isfinite(lpos_x_m) && isfinite(lpos_y_m)) {
        float cur_err_m = hypotf(lpos_x_m - frontier_target_x_m,
                                 lpos_y_m - frontier_target_y_m);
        if (frontier_progress_check_ms == 0) {
          // First sample — seed baseline, start the clock
          frontier_progress_err_m = cur_err_m;
          frontier_progress_check_ms = t;
        } else {
          // Track best error seen so far
          if (cur_err_m < frontier_progress_err_m)
            frontier_progress_err_m = cur_err_m;

          // Check at interval: if current error is worse than best + margin, abort
          if ((t - frontier_progress_check_ms) >= FRONTIER_PROGRESS_CHECK_MS) {
            if (cur_err_m > (frontier_progress_err_m + FRONTIER_PROGRESS_MIN_GAIN_M)) {
              printf("FRONTIER: no progress err=%.2f best=%.2f (%.1fs) -> abort\n",
                     cur_err_m, frontier_progress_err_m,
                     (float)(t - frontier_progress_check_ms) * 0.001f);
              frontier_move_fail_count++;
              frontier_reset_move_target();
              if (frontier_move_fail_count >= FRONTIER_MAX_MOVE_FAILURES) {
                enter_state(ST_LANDING, "No Progress");
              } else {
                frontier_restart_relock_window(t, NAN, NAN);
                enter_state(ST_HOVER_SCAN, "No Progress Retry");
              }
              break;
            }
            // Reset timer for next interval check
            frontier_progress_check_ms = t;
          }
        }
      }

      if ((t - frontier_state_enter_ms) >= FRONTIER_MOVE_TIMEOUT_MS) {
        float err_m = NAN;
        float speed_mps = NAN;
        if (lpos_fresh(t) && isfinite(lpos_x_m) && isfinite(lpos_y_m) &&
            isfinite(lpos_vx_mps) && isfinite(lpos_vy_mps)) {
          err_m = hypotf(lpos_x_m - frontier_target_x_m,
                         lpos_y_m - frontier_target_y_m);
          speed_mps = hypotf(lpos_vx_mps, lpos_vy_mps);
        }

        frontier_move_fail_count++;
        printf("FRONTIER: move timeout target=(%.2f,%.2f) err=%.2f speed=%.2f failures=%u/%u\n",
               frontier_target_x_m, frontier_target_y_m,
               err_m, speed_mps,
               (unsigned)frontier_move_fail_count,
               (unsigned)FRONTIER_MAX_MOVE_FAILURES);

        frontier_reset_move_target();
        if (frontier_move_fail_count >= FRONTIER_MAX_MOVE_FAILURES) {
          enter_state(ST_LANDING, "Move Timeout");
        } else {
          frontier_restart_relock_window(t, NAN, NAN);
          enter_state(ST_HOVER_SCAN, "Move Retry");
        }
        break;
      }
      ...
      if (lpos_fresh(t) && isfinite(lpos_x_m) && isfinite(lpos_y_m) &&
        ...
        float err_m = hypotf(lpos_x_m - frontier_target_x_m,
                             lpos_y_m - frontier_target_y_m);
        float speed_mps = hypotf(lpos_vx_mps, lpos_vy_mps);
        if (err_m <= FRONTIER_ARRIVAL_TOL_M && speed_mps <= FRONTIER_MAX_SETTLE_SPEED_MPS) {
          frontier_move_count++;
          frontier_move_fail_count = 0;
          frontier_restart_relock_window(t, frontier_target_x_m, frontier_target_y_m);
          printf("FRONTIER: move success err=%.2f speed=%.2f moves=%u\n",
                 err_m, speed_mps, (unsigned)frontier_move_count);
          enter_state(ST_SETTLE, "Move Complete");
        }
      }
    } break;
    ...
    case ST_SETTLE: {
      ...
      if (!hover_xy_locked) break;
      if ((t - frontier_state_enter_ms) < FRONTIER_SETTLE_MS) break;
      if (!scan_fresh(t) || frontier_last_map_scan_ms == 0) break;

      char reasons[160];
      if (!frontier_motion_ready(t, true, reasons, sizeof(reasons))) break;

      enter_state(ST_HOVER_SCAN, "Settled");
    } break;
...
static void enter_state(State ns, const char* reason) {
  ...
  bool leaving_hover = state_is_hover_like(st) && !state_is_hover_like(ns);
  bool entering_hover = !state_is_hover_like(st) && state_is_hover_like(ns);
  ...
  if (state_is_frontier(ns)) {
    frontier_state_enter_ms = now_ms();
    if (!state_is_frontier(st)) {
      frontier_reset_session(frontier_state_enter_ms);
      float map_cx_m = isfinite(lpos_x_m) ? lpos_x_m : 0.0f;
      float map_cy_m = isfinite(lpos_y_m) ? lpos_y_m : 0.0f;
      frontier_reset_map(map_cx_m, map_cy_m);
      printf("FRONTIER: session start mode=%s origin=(%.2f,%.2f)\n",
             nav_mode == NAV_MODE_FRONTIER_BASIC ? "frontier" : "hover_only",
             map_cx_m, map_cy_m);
    }
  }
  if (ns == ST_LANDING) {
...
static bool takeoff_brake_settled(uint64_t t) {
  ...
  // Wall-clearance gate: don't settle if airwall would be active at handoff,
  // because the repulsion pushes will keep vy oscillating above XY lock threshold.
  {
    float aw_vx_tmp = 0.0f, aw_vy_tmp = 0.0f;
    if (airwall_repulsion(t, &aw_vx_tmp, &aw_vy_tmp)) {
      takeoff_brake_settled_since_ms = 0;
      return false;
    }
  }
...
static void control_tick(void) {
  ...
  if (fc_armed && have_att && have_yaw_target && (st == ST_TAKEOFF || state_is_hover_like(st))) {
    ...
    if (fabsf(yaw_err) > YAW_DRIFT_ACCEPT_DEG && t >= yaw_spin_suppress_until_ms) {
      ...
      if (yaw_drift_last_strike_ms != 0 &&
        ...
        if (state_is_hover_like(st) && hover_xy_locked) {
          hover_xy_locked = false;
          hover_xy_lock_ms = 0;
          xy_lock_candidate_active = false;
          xy_lock_candidate_since_ms = 0;
  ...
  if (fc_armed && (st == ST_TAKEOFF || state_is_hover_like(st))) {
    HoverPhase phase = current_hover_phase();
  ...
  if (fc_armed && state_is_frontier(st)) {
    char abort_reason[160];
    if (frontier_abort_reason(t, abort_reason, sizeof(abort_reason))) {
      printf("FRONTIER: abort reason=%s\n", abort_reason[0] ? abort_reason : "unknown");
      enter_state(ST_LANDING, abort_reason[0] ? abort_reason : "Frontier Abort");
      goto control_tick_done;
    }
  }
  ...
  switch (st) {
    ...
    case ST_TAKEOFF: {
      ...
      if (takeoff_brake_active) {
        ...
        if (takeoff_brake_settled(t)) {
          ...
          enter_state(nav_mode == NAV_MODE_FRONTIER_BASIC ? ST_HOVER_SCAN : ST_HOVER, "Brake Settled");
          break;
        }
    ...
    case ST_MOVE_TO_FRONTIER:
    case ST_SETTLE:
      if (nav_mode != NAV_MODE_FRONTIER_BASIC) {
        enter_state(ST_HOVER, "Frontier Disabled");
        break;
      }
      frontier_tick(t);
      sync_hover_phase(t);
    ...
    case ST_LANDING: {
      ...
      // WORLD vertical descent — with air-wall repulsion to avoid
      // drifting into a wall during the descent phase.
      {
        float aw_vx = 0.0f, aw_vy = 0.0f;
        airwall_repulsion(t, &aw_vx, &aw_vy);
        send_vel_frame(aw_vx, aw_vy, +0.15f, 0, MAV_FRAME_LOCAL_NED);
      }
  ...
  if (ceiling_override_pending && fc_armed &&
      (st == ST_TAKEOFF || state_is_hover_like(st))) {
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--hover-only") == 0) {
      nav_mode = NAV_MODE_HOVER_ONLY;
      continue;
    }
    if (strcmp(argv[i], "--frontier") == 0) {
      nav_mode = NAV_MODE_FRONTIER_BASIC;
      continue;
    }
  ...
  printf("MODE=%s\n", nav_mode == NAV_MODE_FRONTIER_BASIC ? "FRONTIER_BASIC" : "HOVER_ONLY");
