...
MissionOutput mission_state_machine_tick(MissionStateMachine*m,const MissionInput*i,const AutonomyConfig*c){
  ...
  switch(m->state){
    ...
    case MISSION_PREFLIGHT_CHECK:if(i->preflight_valid&&i->start_requested){enter(m,MISSION_WAIT_HEARTBEAT,i->now_ms);reason="preflight_passed";}else if(timed_out(m,i->now_ms,15000)){enter(m,MISSION_FAILSAFE,i->now_ms);reason="preflight_timeout";}break;
    ...
    case MISSION_HOVER_SCAN:action=ACTION_HOVER;if(i->scan_fresh&&timed_out(m,i->now_ms,(uint64_t)(c->hover_settle_time_s*1000.0f))){if(i->exploration_enabled){enter(m,MISSION_SELECT_FRONTIER,i->now_ms);reason="fresh_scan";}else{enter(m,MISSION_HOLD,i->now_ms);reason="exploration_disabled";}}else if(!i->scan_fresh&&timed_out(m,i->now_ms,(uint64_t)(c->transient_hold_timeout_s*1000.0f))){enter(m,MISSION_HOLD,i->now_ms);reason="scan_stale";}break;
    ...
    case MISSION_LAND:action=ACTION_LAND;if(i->landed_observed){enter(m,MISSION_DISARM,i->now_ms);action=ACTION_DISARM_IF_LANDED;reason="touchdown_confirmed";}else if(timed_out(m,i->now_ms,20000)){enter(m,MISSION_FAILSAFE,i->now_ms);action=ACTION_LAND;reason="landing_timeout_retry_failsafe";}break;
    case MISSION_DISARM:action=ACTION_DISARM_IF_LANDED;if(!i->landed_observed){enter(m,MISSION_LAND,i->now_ms);action=ACTION_LAND;reason="lost_landed_confirmation";}else if(i->disarmed_observed){enter(m,MISSION_COMPLETE,i->now_ms);action=ACTION_CLOSE;reason="disarmed_observed";}else if(timed_out(m,i->now_ms,5000)){enter(m,MISSION_LAND,i->now_ms);action=ACTION_LAND;reason="disarm_observation_timeout";}break;
