...
ROOM_COLORS = [
    ...
    ('red', (220, 70, 70, 220)),
    ('green', (60, 190, 90, 220)),
    ('orange', (235, 150, 40, 220)),
    ('purple', (165, 90, 220, 220)),
    ('white', (235, 235, 240, 220)),
]
...
def load_room_preset(logfile):
    ...
    try:
        preset = {
            ...
            'color_idx': int(room.get('color_idx', 0)),
        }
    except (KeyError, TypeError, ValueError) as exc:
        print(f"Warning: invalid room preset {preset_path}: {exc}")
        return None, preset_path

    preset['color_idx'] = max(0, preset['color_idx']) % len(ROOM_COLORS)
...
def save_room_preset(logfile, room_state):
    ...
    payload = {
        ...
        'room': {
            ...
            'color_idx': int(room_state.get('color_idx', 0)) % len(ROOM_COLORS),
        },
    }
...
def launch_viewer(records, max_points, logfile, initial_start_frame=0,
    ...
    def rebuild_room_mesh():
        ...
        if room_lines:
            _, rgba = ROOM_COLORS[int(room_state.get('color_idx', 0)) % len(ROOM_COLORS)]
    ...
    track_gui(Text(
        text=(
            ...
            '1/2: point size   C: colour   Shift+C: room colour   T: traj   G: ground   R: room\n'
    ...
    def update_room_info():
        if room_state is None:
            ...
            return
        room_color_name = ROOM_COLORS[int(room_state.get('color_idx', 0)) % len(ROOM_COLORS)][0]
        room_info.text = (
    ...
    class KeyHandler(Entity):
        def input(self, key):
            if key == '1':
                ...
                if held_keys['shift'] and room_state is not None:
                    room_state['color_idx'] = (
                        int(room_state.get('color_idx', 0)) + 1
                    ) % len(ROOM_COLORS)
                    rebuild_room_mesh()
                    update_room_info()
                else:
