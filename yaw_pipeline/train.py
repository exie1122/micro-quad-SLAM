...
class PairDataset(Dataset):
    ...
    def __init__(self, pairs, img_size, frame_cache):
        self.pairs = pairs
        self.size = img_size
    ...
    def _img(self, path):
        if path not in self.cache:
            ...
            if im.shape != (self.size, self.size):
    ...
    def __getitem__(self, i):
        p = self.pairs[i]
        a = self._img(p["img_a"]).astype(np.float32) / 255.0
        b = self._img(p["img_b"]).astype(np.float32) / 255.0
        x = np.stack([a, b], 0)
        y = np.array([np.sin(p["rel_yaw"]), np.cos(p["rel_yaw"])], dtype=np.float32)
...
def main():
    ...
    train_ld = DataLoader(PairDataset(train_pairs, size, cache), batch_size=tc["batch_size"],
                          shuffle=True, num_workers=0)
    val_ld = DataLoader(PairDataset(val_pairs, size, cache), batch_size=tc["batch_size"],
