...
| `logs/scanlog.bin` | 156,655 | 244 | 75,415–123,044 | clean single session; identical to external `floor1_scanlog.bin` |
...
- `/home/ethan/year1_scf_drone/data/scanlogs/` duplicates the named committed
  ...
  - `floor1_scanlog.bin`: 156,655 bytes, 244 clean SCN3 records; byte-identical to
    `log/logs/scanlog.bin` in Git.
...
The external `FIGURE_REPRODUCTION.md` reports that `occupancy_*` and scanlog
statistics can be regenerated from archived SCLOG3 files. The specific raw input
...
SCLOG3 plus DataFlash evidence exists. A narrower prior claim remains true: the
specific SCLOG3 inputs formerly referenced as `/home/ethan/scanlog.bin` and
`/home/ethan/CLEAN_scanlog.bin` for some publication plots cannot be uniquely
matched; the large archived `scanlog.bin` is SCLOG1 and must not be substituted.
