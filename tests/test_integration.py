...
def main():
    ...
    with tempfile.TemporaryDirectory(dir=ROOT / "runs") as td:
        ...
        assert "explicit --tof-protocol" in result.stdout

    with tempfile.TemporaryDirectory(dir=ROOT / "runs") as td:
        result = run("--backend", "mavlink", "--allow-live-serial", "--serial",
                     "/dev/definitely-not-opened", "--tof-serial", "/dev/also-not-opened",
                     "--tof-protocol", "legacy-a5-v0", "--map-source", "live", "--run-dir", td)
        assert result.returncode != 0
        assert "unable to configure/open MAVLink" in result.stdout

    makefile = (ROOT / "Makefile").read_text()
    launcher = (ROOT / "scripts" / "run_autonomy.sh").read_text()
    legacy = (ROOT / "c" / "frontier.c").read_text()
    service = (ROOT / "deploy" / "micro-quad-autonomy.service").read_text()
    service_runner = (ROOT / "deploy" / "run_live_service.sh").read_text()
    live_config = (ROOT / "config" / "autonomy-live.conf").read_text()
    ...
    assert "run_live_service.sh" in service and "frontier" not in service
    assert '"$AUTONOMY_BIN"' in service_runner and "frontier.c" not in service_runner
    assert "AUTONOMY_BIN=/opt/micro-quad-autonomy/bin/autonomy_controller" in live_config
    args_line = next(line for line in service_runner.splitlines() if line.startswith('args='))
    assert "--start-mission" not in args_line
    assert "ALLOW_LIVE_SERIAL=no" in live_config and "EXPLORATION_ENABLED=no" in live_config
